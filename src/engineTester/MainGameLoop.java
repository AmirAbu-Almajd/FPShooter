...
public class MainGameLoop {
	public static void main(String[] args) {
		...
		TexturedModel start_tex = new TexturedModel(start_raw,
				new ModelTexture(loader.loadTexture("start.jpg", "JPG")));
		TexturedModel controls_tex = new TexturedModel(controls_raw,
				new ModelTexture(loader.loadTexture("controls.jpg", "JPG")));
		TexturedModel controlsbg_tex = new TexturedModel(controlsbg_raw,
				new ModelTexture(loader.loadTexture("controls background.jpg", "JPG")));
		TexturedModel carTEx = new TexturedModel(car2, new ModelTexture(loader.loadTexture("dp2.png", "PNG")));
		TexturedModel health_tex = new TexturedModel(health_raw,
				new ModelTexture(loader.loadTexture("green.png", "PNG")));
		// entities initialization

		Entity entity = matrices.load2D(matrices.vertices, matrices.textureCoords, matrices.indices, "white2.png",
				"PNG");
...
//		}
		...
		while (!Display.isCloseRequested()) {
			...
			if (screen == 1) {
				...
				if(!weaponPicked) {
					Weapons.QBZ.setPosition(new Vector3f(-15,1.5f,15));
					Weapons.QBZ.increaseRotation(0, 1, 0);
				...
				if (Mouse.isButtonDown(0)) {
					...
					Weapons.bullet.setPosition(new Vector3f(camera.getPosition().x - x2, camera.getPosition().y,
							camera.getPosition().z - z2));
